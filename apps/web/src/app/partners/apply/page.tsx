            ...
            {[
              ...
              <div key={link.label} className="mb-3 last:mb-0">
                ...
                >
                  ...
                  <span className="text-malama-ink-faint group-hover:text-malama-accent transition-colors shrink-0">Copy</span>
                </button>
              </div>
            ))}
          </div>
          <Link href="/partners" className="inline-flex items-center gap-2 text-malama-accent text-sm font-black hover:underline">
            Back to Partners <ArrowRight size={14} />
          </Link>
        </motion.div>
      </div>
    )
    ...
    <div className="min-h-screen bg-malama-bg">
      <div className="max-w-2xl mx-auto px-6 pt-20 pb-20">
        <motion.div initial="hidden" animate="show" variants={fadeUp} custom={0} className="mb-10">
          ...
          <p className="text-xs font-black uppercase tracking-widest text-malama-accent mb-3">Partner Application</p>
          <h1 className="text-4xl font-serif font-black text-white tracking-tight mb-3">Join the program</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <motion.div initial="hidden" animate="show" variants={fadeUp} custom={1}>
            ...
            <input required value={form.displayName} onChange={set('displayName')} placeholder="Your name"
          ...
          <motion.div initial="hidden" animate="show" variants={fadeUp} custom={3}>
            ...
            <input required value={form.walletAddress} onChange={set('walletAddress')} placeholder="0x..."
            ...
            <p className="text-[11px] text-malama-ink-faint mt-1">USDC commissions are paid to this address on Base network.</p>
          </motion.div>
          ...
          <motion.div initial="hidden" animate="show" variants={fadeUp} custom={4}>
            ...
            <div className="relative">
              ...
              <input value={form.twitterHandle} onChange={set('twitterHandle')} placeholder="yourhandle"
          ...
          <motion.div initial="hidden" animate="show" variants={fadeUp} custom={6}>
            ...
            <textarea required rows={4} value={form.promoMethod} onChange={set('promoMethod')}
              placeholder="e.g. Twitter thread campaign, YouTube video, newsletter, Telegram group..."
              className="w-full bg-malama-card border border-malama-line rounded-malama px-4 py-3 text-sm text-malama-ink placeholder-malama-ink-faint focus:outline-none focus:border-malama-accent/50 transition-colors resize-none" />
          </motion.div>
