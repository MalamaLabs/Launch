...
function SignInForm() {
  ...
  const [evmConnectStarted, setEvmConnectStarted] = useState(false)
        ...
        <form onSubmit={signInWithEmail} className="mb-6 space-y-3 text-left">
          <label className="block">
            ...
            <input type="email" autoComplete="email" value={email}
              ...
              placeholder="you@example.com" disabled={emailSubmitting}
          ...
          <button type="submit" disabled={emailSubmitting || !emailOk || !magic}
            ...
            {emailSubmitting ? <><Loader2 className="h-5 w-5 animate-spin" /> Signing in…</> : <><Mail className="h-5 w-5" /> Continue with email</>}
          </button>
          {!magic && <p className="text-center text-xs text-yellow-500/80">Email sign-in requires <code>NEXT_PUBLIC_MAGIC_API_KEY</code> to be configured.</p>}
        </form>
...
export default function SignInPage() {
  return (
    <Suspense fallback={<div className="flex min-h-screen items-center justify-center bg-black"><Loader2 className="h-8 w-8 animate-spin text-malama-accent" /></div>}>
      <SignInForm />
    </Suspense>
  )
}
